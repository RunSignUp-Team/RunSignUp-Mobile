        ...
        if([[rootXML tag] isEqualToString:@"races"]){
            ...
            if(races != nil){
                for(RXMLElement *race in races){
                    ...
                    RXMLElement *raceDate = [race child:@"next_date"];
                    if(raceName != nil && raceID != nil && raceDate != nil){
                        NSString *curtailedName = [raceName text];
                        if([curtailedName length] > 34)
                            curtailedName = [curtailedName substringToIndex: 33];
                        NSDictionary *dict = [[NSDictionary alloc] initWithObjectsAndKeys:curtailedName,@"Name",[raceID text],@"RaceID",[raceDate text],@"Date",nil];
